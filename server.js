...
// Inicializa Mercado Pago (Produção) com SDK v2+
const mp = new mercadopago({
  access_token: process.env.MP_ACCESS_TOKEN, // PROD-xxx
});
...
// Webhook Mercado Pago (produção) com validação HMAC
app.post('/webhook', express.raw({ type: '*/*' }), async (req, res) => {
  ...
  try {
    ...
    if (paymentId) {
      const paymentDetails = await mp.payment.get(paymentId);
